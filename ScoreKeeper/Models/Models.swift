...
enum SchemaV3: VersionedSchema {
    ...
    // The default values on id/name matter: without them, lightweight migration
    // cannot infer how to add Game.id to V2 stores (which have no id), so V2
    // users fail to load entirely. Defaults do not change the version checksum,
    // so existing V3 stores are unaffected.
    @Model
    class Player: Identifiable, Hashable {
